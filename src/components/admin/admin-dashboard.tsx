...
export function AdminDashboard() {
  ...
  const [activeSection, setActiveSection] = useState<"orders" | "products">(
    "orders",
  );
  ...
  const loadOrders = useCallback(
    async (options?: { cursor?: string; append?: boolean }) => {
      ...
      try {
        ...
        const payload = await readAdminEnvelope<
          AdminOrderSummary[],
          OrdersMeta
        >(
  ...
  useEffect(() => {
    if (authState === "signed-in" && activeSection === "orders") {
      void loadOrders();
    }
  }, [activeSection, authState, loadOrders]);
  ...
  async function login(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      await readAdminEnvelope<{ authenticated: true }>(
  ...
  async function openOrder(token: string) {
    ...
    setNotice(null);
    try {
      const payload = await readAdminEnvelope<AdminOrderDetail>(
  ...
  async function deleteOrder() {
    ...
    const stockMessage =
      ...
      selectedOrder.status === "WAITING_REVIEW"
        ? "اگر موجودی این سفارش هنوز رزرو باشد، به موجودی محصولات برمی‌گردد."
        : "موجودی محصولات پرداخت‌شده تغییر نمی‌کند.";
    if (
      !window.confirm(
        `این سفارش، اقلام، اطلاعات پرداخت و رسید آن برای همیشه حذف می‌شوند. ${stockMessage}\n\nاز حذف کامل سفارش مطمئن هستید؟`,
      )
    ) {
      return;
    }
    ...
    setNotice(null);
    try {
      await readAdminEnvelope<{ deleted: true; publicToken: string }>(
        await fetch(
          `/api/admin/orders/${encodeURIComponent(selectedOrder.publicToken)}`,
          { method: "DELETE" },
        ),
      );
      setSelectedOrder(null);
      setReviewNote("");
      setNotice("سفارش و اطلاعات پرداخت وابسته به آن حذف شد.");
      await loadOrders();
    } catch (reason: unknown) {
      if (reason instanceof AdminApiError && reason.status === 401) {
        returnToLogin(reason.message);
        ...
        setError(
          reason instanceof Error ? reason.message : "حذف سفارش انجام نشد.",
        );
      }
    } finally {
      setIsDetailBusy(false);
    ...
    <main className="min-h-screen">
      ...
      <div className="mx-auto max-w-[96rem] px-5 py-8 sm:px-8 sm:py-12">
        <nav
          ...
          className="bg-surface border-border/70 mb-8 grid gap-2 rounded-xl border p-2 sm:inline-grid sm:grid-cols-2"
        >
          <button
            type="button"
            aria-current={activeSection === "orders" ? "page" : undefined}
            className={cn(
              "flex min-h-12 items-center justify-center gap-2 rounded-lg px-5 text-sm font-medium transition-colors",
              activeSection === "orders"
                ? "bg-[#121210] text-[#fffdf9]"
        ...
        {activeSection === "products" ? (
          <AdminProductsPanel onUnauthorized={returnToLogin} />
        ) : (
          <>
                ...
                {!isBusy && orders.length === 0 ? (
                  <div className="bg-surface border-border/70 rounded-xl border px-6 py-16 text-center">
                    ...
                    <p className="text-muted-foreground mt-2 text-sm">
                      فیلترها را تغییر دهید یا پس از ثبت سفارش دوباره بررسی
                      کنید.
                    </p>
                  </div>
                ) : null}
                ...
                {isDetailBusy && !selectedOrder ? (
                  ...
                  <div>
                    ...
                    <div className="mt-5 flex flex-wrap gap-2">
                      ...
                      <StatusBadge
                        tone={paymentTone(selectedOrder.paymentStatus)}
                      >
                    ...
                    <dl className="border-border/70 mt-6 space-y-3 border-y py-5 text-sm">
                      ...
                      <div className="flex justify-between gap-4">
                        ...
                        <dd>
                          {selectedOrder.shippingMethod === "POST"
                            ? selectedOrder.shippingCost.rial
                            : "پرداخت درب منزل"}
                        </dd>
                      </div>
                    </dl>
                      ...
                      {selectedOrder.items.map((item) => (
                        ...
                        >
                          ...
                          <div className="text-muted-foreground mt-2 flex justify-between gap-4">
                            <span>
                              تعداد {formatPersianInteger(item.quantity)}
                            </span>
                    ...
                    <div className="border-danger/15 mt-8 border-t pt-6">
                      <h3 className="text-danger text-sm font-semibold">
                        حذف سفارش
                      </h3>
                      <p className="text-muted-foreground mt-2 text-xs leading-6">
                        این عملیات سفارش، اقلام، پرداخت و رسید ذخیره‌شده را برای
                        همیشه حذف می‌کند و قابل بازگشت نیست.
                      </p>
                      <Button
                        type="button"
                        variant="destructive"
